...
class ToyTool : public Tool
{
  ...
  ToyTool () : Tool ("ToyTool")
  {
    ...
    getParser()->push_front (
      new OptionOneParam (
        STR_RANGE_MIN, //argument name
        "lower range bound", //argument short help message
        false, //mandatory?
        "1" //default value
      ));
    getParser()->push_front (
    ...
    // Note: if the mandatory argument is not provided, Tool terminates with an error
    // on stderr.
  }

  // The 'execute' method must be implemented as we are a subclass of Tool.
  // This method does the actual job our tool is supposed to do 
  // (here we just do some dummy computation)
  void execute ()
  {
    ...
    // Note how we use the Tool::createIterator method. 
    // According to the value of the "-verbose" argument,
    ...
    // So, as you an see, Tool provides with a convenient way to monitor your software.
    getTimeInfo().start ("computation");

    // We iterate the range through the Dispatcher we got from our Tool parent class.
    // The dispatcher is configured with the number of cores provided by the "-nb-cores"
    ...
    // We gather some statistics. Note how we use the getInfo() method from the 
    // parent class, Tool.
    // If the verbosity is *not* 0, all this information will be dumped in the 
    // console at the end of the tool execution.
