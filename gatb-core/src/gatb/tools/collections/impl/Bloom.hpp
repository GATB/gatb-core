...
public:
    ...
    BloomMinimizerCoherent (u_int64_t tai_bloom, size_t kmersize, size_t nbHash = 7, size_t block_nbits = 9, size_t minimizer_block_nbits = 12)
        : Bloom<Item> (tai_bloom + (1<<minimizer_block_nbits), nbHash), _nbits_BlockSize(block_nbits), _nbits_MinimizerBlockSize(minimizer_block_nbits), _kmerSize(kmersize)
    {
 ...
 * implementation should be used only with Item being a kmer.
 */
template <typename Item> class BloomMinimizerCoherent2 : public Bloom<Item>
{
public:
     ...
     * \param[in] nbHash : number of hash functions to use
     * \param[in] block_nbits : size of the block (actual 2^nbits)
     * \param[in] mmersize : minimizer size
     * \param[in] large_block_nbits : size of the minimizer block (actual 2^nbits) */
    BloomMinimizerCoherent2 (u_int64_t tai_bloom, size_t kmersize, size_t nbHash = 7, size_t block_nbits = 9, size_t mmersize = 16, size_t large_block_nbits = 12)
        : Bloom<Item> (tai_bloom + (1<<large_block_nbits), nbHash), _nbits_BlockSize(block_nbits), _nbits_LargeBlockSize(large_block_nbits), _kmerSize(kmersize), _mmerSize(mmersize)
    {
        // TODO : check block_nbits <= large_block_nbits

        _mask_block = (1<<_nbits_BlockSize) - 1;
        _large_mask_block = (1<<_nbits_LargeBlockSize) - 1;
        _reduced_tai = this->tai - (1<<_nbits_LargeBlockSize);
        ...
        _maskkm2  = (un << ((_kmerSize-2)*2)) - un;
        _kmerMask = (un << (_kmerSize*2))     - un;
        _mmerMask = (un << (_mmerSize*2))     - un;

        Item trois = 3;

        _prefmask = trois << ((_kmerSize-1)*2); //bug was here 3 instead of item trois

        _mmerCount = (_kmerSize - 2) - _mmerSize + 1; // (_kmerSize - 2) <=> Hashpart Size

        _hashpart = _maskkm2 + un; // > max value
        _minimizer = _mmerMask + un; // > max value
        _position = -1;
    }

    void begin() 
    {
        _hashpart = _maskkm2 + 1; // > max value
        _position = -1;
    }
    ...
    void insert (const Item& item)
    {
        ...
        computeMinimizer(hashpart);
        h0 = this->_hash(_minimizer,0) % _reduced_tai;
        __sync_fetch_and_or (this->blooma + (h0 >> 3), bit_mask[h0 & 7]);

        pre_h1 = this->_hash(hashpart, 1) + pref_val;
        h1 = h0 + (pre_h1 & _large_mask_block);
        __sync_fetch_and_or (this->blooma + (h1 >> 3), bit_mask[h1 & 7]);
    ...
    /** \copydoc IBloom::getName*/
    std::string  getName () const { return "minimizer2"; }
    ...
    bool contains (const Item& item)
    {
        ...
        if (_hashpart != hashpart)
        {
            ...
            {
                computeMinimizer(hashpart);
                _minimizerHash = this->_hash(_minimizer,0);
            }
            else
            {
                Item mmer = hashpart & _mmerMask;
                Item revMmer = revcomp(mmer, _mmerSize);
                if (revMmer < mmer) mmer = revMmer;

                if (mmer <= _minimizer) {
                    _minimizer = mmer;
                    _minimizerHash = this->_hash(_minimizer,0);
                    _position = _mmerCount - 1;
                }
            }
            ...
            _hashpart = hashpart;
            _hashpartHash = this->_hash(_hashpart, 1);
        }
        ...
        h0 = _minimizerHash % _reduced_tai;
        pre_h1 = _hashpartHash + pref_val;
        h1 = h0 + (pre_h1 & _large_mask_block);
...
private:
    u_int64_t _mask_block;
    size_t    _nbits_BlockSize;
    u_int64_t _large_mask_block;
    size_t    _nbits_LargeBlockSize;
    u_int64_t _reduced_tai;
    ...
    u_int64_t _minimizerHash;
    int16_t _position;

    void computeMinimizer(const Item& hashpart) {
        _minimizer = _mmerMask + 1; // Max value
        _position = -1;

        for (size_t i=0; i<_mmerCount; i++) {
            Item mmer = (hashpart >> (2*i)) & _mmerMask;
            Item rev = revcomp(mmer, _mmerSize);
            if (rev < mmer) mmer = rev;

            if (mmer < _minimizer) {
                _minimizer = mmer;
                _position = _mmerCount - i - 1;
            }
        }
    }
};
