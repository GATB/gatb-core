...
#ifndef DONTMARK
        if(nbNodes>1 || len_right>0){ //len_right==0 && nbNodes==1 = particular case of the first contig of size kmer-size (== L), contig already marked (before the loop), do not mark again, this will stop extension
            ...
            if (already_seen)
            {
                INFO (("... XXX not extending node %s because last k-1-mer was already seen\n", seq.toString().c_str()));
                continue;
            }
        }
#endif
