    ...
    struct mmap_memory_model {
        ...
        struct sorter {
                ...
                , m_kf(kf)
            {
                ...
                // I think this is an in-place partitioning algorithms in linear time.
                // starting from the last cursor,
                // for each element in a cursor,
                // if this element is not supposed to be in that cursor,
                // then find the cursor where it should be, and select the current element
                // and swap those two elements (invariant: one of the two swapped elements is never swapped again)
